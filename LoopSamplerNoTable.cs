...
namespace CrawfisSoftware.PCG
{
    ...
    /// </summary>
    public class LoopSamplerNoTable
    {
        ...
        public LoopSamplerNoTable(int width, int height, Random random,
            ...
            while (currentAttempt < MAX_ATTEMPTS)
            {
                try
                {
                    ...
                    // IList<short> rowLists =
                        ...
                        //     .ToList();
                    // var list = ValidPathRowEnumerator.ValidRows(_width, inflow).ToList();
                    // int outflowCandidate = list[_random.Next(0, list.Count)];
                    int outflowCandidate = RandomEvenBitPattern(_width, _random);
                    int horizontalSpans;
                    while (!ValidateAndUpdateComponents(inflow, outflowCandidate, components, 0,
                               out horizontalSpans, 1))
                    {
                        outflowCandidate = outflowCandidate = RandomEvenBitPattern(_width, _random);
                    }
                    ...
                    for (int currentRow = 1; currentRow < _height - 2; currentRow++)
                    {
                        ...
                        //rowLists = //validpathrowEnumerator.ValidRowList(_tableWidth, inflow).ToList();
                        var list = ValidPathRowEnumerator.ValidRows(_width, inflow).ToList();
                        outflowCandidate = list[_random.Next(0, list.Count)];
                        while (!(ValidateAndUpdateComponents(inflow, outflowCandidate, components, currentRow,
                                   out horizontalSpans, 0)))
                        {
                            outflowCandidate = list[_random.Next(0, list.Count)];
                        }
        ...
        private IList<IList<int>> InitializeComponents(int firstRow)
        {
            ...
            for (int i = 0; i < _height; i++)
                components[i] = new int[_tableWidth];
            IList<int> inflowList = ValidPathRowEnumerator.InflowsFromBits(_tableWidth, firstRow);
            int counter = 0;
        ...
        {
            List<int> previousInflows = ValidPathRowEnumerator.InflowsFromBits(_tableWidth, previousInflow);
            
            int min = previousInflows.Min();
            int max = previousInflows.Max();
            
            IList<int> inflowList = ValidPathRowEnumerator.InflowsFromBits(_tableWidth, inflow);
