...
namespace CrawfisSoftware.PCG
{
    ...
    /// </summary>
    public class PathSamplerArbitraryNew
    {
        ...
        public PathSamplerArbitraryNew(int width, int height, Random random,
            ...
            Sample((int row, int column) start, (int row, int column) end)
        {
            ...
            for (int currentRow = 1; currentRow < _height - 2; currentRow++)
            {
                ...
                if (currentRow == start.row)
                {
                    ...
                    if (IsBitSet(inflow, start.column))
                    {
                        ...
                        outflowCandidate = rowLists[_random.Next(rowLists.Count - 1)];
                        while (!ValidateAndUpdateComponents(alteredInflow, outflowCandidate, components, currentRow,
                                   out horizontalSpans, 1) ||
                ...
                if (currentRow == end.row)
                {
                    ...
                    {
                        Console.WriteLine(Convert.ToString(alteredInflow, 2).PadLeft(_width, '0'));
            ...
            while (!lastRowFixed)
            {
                ...
                while (!ValidateAndUpdateComponents(inflow, outflowCandidate, components, secondToLastRow,
                           out horizontalSpans) || NumberOfDistinctValues(components[secondToLastRow + 1]) < 3 ||
